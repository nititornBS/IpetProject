...
{
    [CreateAssetMenu(fileName = "PlanetData", menuName = "TcgEngine/PlanetData", order = 1)]
    public class MapData : ScriptableObject
    {
        ...
        public static List<MapData> planet_list = new List<MapData>();
        ...
        public static void Load(string folder = "")
        {
            if (planet_list.Count == 0)
                planet_list.AddRange(Resources.LoadAll<MapData>(folder));
        }

        public static MapData Get(string id)
        {
            foreach (MapData planet in GetAll())
            {
                if (planet.id == id)
                    return planet;
            }
