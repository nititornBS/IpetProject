...
namespace RogueEngine.UI
{
    public class LobbyPanel : UIPanel
    {
        ...
        {
            game_list.Clear();
            game_list.AddRange(list.items);
            load_panel.Hide();
            no_room_text.enabled = list.items.Length == 0;
        ...
        private void ReceiveRefresh(LobbyGame room)
        {
            ...
            if (joining)
            {
                ...
                if (room.started)
                {
                    ...
                    GameClient.connect_settings.file_host = room.IsHost(udata.id);
                    GameClient.connect_settings.load = room.load;
                    GameClient.connect_settings.server_url = room.game_url;
                    GameClient.connect_settings.game_uid = room.game_uid;
                    GameClient.connect_settings.title = room.title;
                    GameClient.connect_settings.filename = room.filename;
